...
export class WebGPUDirectLightShadowRP {
    ...
    /** @internal */
    private _shadowMatrices: Float32Array = new Float32Array(16 * (WebGPUDirectLightShadowRP._maxCascades));
    /**@internal */
    ...
    update(context: WebGPURenderContext3D): void {
        ...
        for (let i = 0; i < this._cascadeCount; i++) {
            const sliceData: ShadowSliceData = this._shadowSliceDatas[i];
    ...
    render(context: WebGPURenderContext3D, list: WebBaseRenderNode[], count: number): void {
        var shaderValues: WebGPUShaderData = context.sceneData;
        ...
        for (let i = 0, n = this._cascadeCount; i < n; i++) {
            ...
            const shadowCullInfo: ShadowCullInfo = this._shadowCullInfo;
     ...
     * @param camera 
     */
    private _applyRenderData(scene: WebGPUShaderData, camera: WebGPUShaderData) {
        ...
        else scene.removeDefine(Scene3DShaderDeclaration.SHADERDEFINE_SHADOW_CASCADE);
     ...
     * @internal
     */
    private _setupShadowCasterShaderValues(shaderValues: WebGPUShaderData, shadowSliceData: ShadowSliceData, LightParam: Vector3, shadowBias: Vector4): void {
