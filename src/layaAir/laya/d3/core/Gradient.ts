    ...
    */
    ...
    get alphaElements(): Float32Array {
        if (this._colorAlphaKeysCount < 2) {
            ...
            let alphaElements = this.__alphaElements;
            alphaElements[1] = alphaElements[5] = this._alphaElements[1];
            alphaElements[2] = alphaElements[6] = this._alphaElements[2];
            alphaElements[3] = alphaElements[7] = this._alphaElements[3];
            alphaElements[0] = 0;
            alphaElements[4] = 1;
            return alphaElements;
        } else {
    ...
    constructor(maxColorRGBKeyCount: number, maxColorAlphaKeyCount: number) {
        ...
        this._alphaElements = new Float32Array(maxColorAlphaKeyCount * 4);
    }
    ...
    addColorAlpha(key: number, value: number): void {
        if (this._colorAlphaKeysCount < this._maxColorAlphaKeysCount) {
            var offset: number = this._colorAlphaKeysCount * 4;
        ...
        if (reverseSearch) {
                ...
                switch (this._mode) {
                    case GradientMode.Blend:
                        if (lerpFactor > left) {
                            ...
                            outColor.a = (x1 * alphaElements[offset + 1] + x2 * alphaElements[offset + 5]) / diff;
                            return curIndex;
                        } else {
                            curIndex--;
                            continue;
                    ...
                    case GradientMode.Fixed:
                        if (lerpFactor > left) {
                            if (lerpFactor > alphaElements[offset + 4])
                                throw "Gradient:wrong startSearchIndex.";
                            outColor.a = alphaElements[offset + 5];
                            return curIndex;
                        } else {
                            curIndex--;
                            continue;
            ...
            for (var i: number = curIndex, n: number = this._alphaElements.length; i < n; i++) {
                ...
                switch (this._mode) {
                    case GradientMode.Blend:
                        if (lerpFactor < right) {
                            var left: number = alphaElements[offset - 4];
                            if (lerpFactor < left)
                                throw "Gradient:wrong startSearchIndex.";
                            var diff: number = right - left;
                            var x1: number = right - lerpFactor;
                            var x2: number = lerpFactor - left;
                            outColor.a = (x1 * alphaElements[offset - 3] + x2 * alphaElements[offset + 1]) / diff;
                            return curIndex;
                        } else {
                            curIndex++;
                            continue;
